...
// --- CONFIGURACIÓN DE FIREBASE (TU BASE DE DATOS REAL) ---
...
// Identificador único para tus datos
    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden relative">
      ...
      <div className="w-16 lg:w-60 bg-slate-900 text-slate-400 flex flex-col z-20 shadow-2xl">
        ...
        <div className="flex-1 py-6 space-y-2 px-2">
            <SidebarButton active={activeTab === 'workstation'} onClick={() => setActiveTab('workstation')} icon={<FileText size={20}/>} label="Informes" />
            <SidebarButton active={activeTab === 'config'} onClick={() => setActiveTab('config')} icon={<Settings size={20}/>} label="Configuración" />
        </div>
        
        <div className="p-4 border-t border-white/5 hidden lg:block">
            <div className="text-[10px] font-bold text-slate-500 uppercase mb-2 italic tracking-widest">v2.0 - LIVE CMS</div>
            ...
            <div className="flex-1 flex flex-col bg-slate-50 relative">
                <div className="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between shrink-0 shadow-sm z-10">
                    <div className="font-bold text-slate-400 text-xs uppercase tracking-widest flex items-center gap-2">
                        <HardDrive size={14}/> Estación de Trabajo
                    </div>
            ...
            <div className="w-80 lg:w-96 bg-white border-l border-slate-200 flex flex-col shadow-2xl z-10 overflow-hidden text-sm">
                ...
                <div className="p-4 border-b border-slate-100">
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Filtrar estudios..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-slate-100 border-transparent rounded-2xl text-xs outline-none focus:bg-white focus:ring-2 ring-indigo-100 transition-all"/>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto bg-white px-2 py-4">
                    {methods.length === 0 && (
                        <div className="p-10 text-center text-slate-300">
                            ...
                            <p className="text-xs font-bold uppercase italic">Sin categorías</p>
                        </div>
                    )}
                    {methods.map(method => {
                        ...
                        return (
                            <div key={method.id} className="mb-1">
                                <button onClick={() => toggleNode(method.id)} className="w-full flex items-center p-3 rounded-xl hover:bg-slate-50 transition-all text-indigo-600 font-black text-[11px] uppercase tracking-widest text-left">
                                        ...
                                        {methodRegions.map(region => {
                                            ...
                                            return (
                                                <div key={region.id} className="ml-3">
                                                    <button onClick={() => toggleNode(regId)} className="w-full flex items-center p-2 rounded-lg hover:bg-slate-50 transition-colors text-slate-700 font-bold text-xs uppercase text-left">
            ...
            <div className="max-w-6xl mx-auto">
               <div className="flex items-center justify-between mb-10">
                   <div>
                       ...
                       <p className="text-sm text-slate-400 font-medium">Configura tus métodos y regiones para organizar las plantillas.</p>
                   </div>
                   <div className="flex gap-2 text-sm">
                       <button onClick={() => { setModalType('method'); setEditData({ order: methods.length }); setIsModalOpen(true); }} className="bg-indigo-600 text-white px-4 py-2 rounded-xl font-bold flex items-center gap-2 shadow-lg"><Plus size={16}/> Nuevo Método</button>
                       <button onClick={() => { setModalType('region'); setEditData({}); setIsModalOpen(true); }} className="bg-slate-800 text-white px-4 py-2 rounded-xl font-bold flex items-center gap-2 shadow-lg"><Plus size={16}/> Nueva Región</button>
                   </div>
               </div>
               ...
               {configSection === 'templates' && (
                  <div className="space-y-6">
                      <div className="flex justify-between items-center bg-white p-6 rounded-3xl border border-slate-200">
                          ...
                          <button onClick={() => { setModalType('template'); setEditData({ methodId: methods[0]?.id, regionId: regions[0]?.id }); setIsModalOpen(true); }} className="bg-indigo-600 text-white px-6 py-3 rounded-2xl font-black text-sm shadow-xl flex items-center gap-2"><Plus size={18}/> Crear Plantilla</button>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {templates.map(t => (
                              <div key={t.id} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:border-indigo-300 transition-all group flex flex-col h-64">
                                  ...
                                  <p className="text-[10px] text-slate-400 line-clamp-6 font-mono bg-slate-50 p-4 rounded-2xl flex-1">{t.content}</p>
                              </div>
                          ))}
                      </div>
                  </div>
                           ...
                           <div className="space-y-3">
                               {methods.map(m => (
                                   <div key={m.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl group">
                                       ...
                                       <button onClick={() => deleteItem('method', m.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100"><Trash2 size={16}/></button>
                                   </div>
                               ))}
                           </div>
                       </div>
                           ...
                           <div className="space-y-3">
                               {regions.map(r => (
                                   <div key={r.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl group">
                                       ...
                                       <button onClick={() => deleteItem('region', r.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100"><Trash2 size={16}/></button>
                                   </div>
                               ))}
                           </div>
                       </div>
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                  <div className="p-8 border-b border-slate-100 flex justify-between items-center">
                      <h3 className="font-black text-xl text-slate-800 uppercase text-sm">Gestión de {modalType === 'method' ? 'Método' : modalType === 'region' ? 'Región' : 'Plantilla'}</h3>
                  ...
                  <div className="p-10 space-y-6">
                      ...
                      {modalType === 'region' && (
                          <div className="space-y-4">
                              ...
                              <input type="text" value={editData.name || ''} onChange={e => setEditData({...editData, name: e.target.value.toUpperCase()})} className="w-full p-5 bg-slate-50 border border-slate-100 rounded-2xl mt-1 outline-none font-bold text-slate-700" placeholder="Ej: NEUROLOGÍA"/>
                          </div>
                      )}
                      ...
                      {modalType === 'template' && (
                          <div className="space-y-4">
                              ...
                              <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Contenido (Pegar texto aquí)</label>
                              <textarea value={editData.content || ''} onChange={e => setEditData({...editData, content: e.target.value})} className="w-full p-6 bg-slate-50 border border-slate-100 rounded-3xl h-64 outline-none font-mono text-[10px] leading-relaxed"/>
                          </div>
                      )}
