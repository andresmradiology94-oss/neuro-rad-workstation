...
// --- COMPONENTE MÓVIL (MODO SECUENCIAL ESTRICTO) ---
const MobileMicInterface = ({ sessionId, user, isOnline }) => {
  const [isListening, setIsListening] = useState(false);
  const [status, setStatus] = useState('Listo');
  const [localText, setLocalText] = useState(''); 
  const recognitionRef = useRef(null);
  const activeSessionRef = useRef(false); // Control manual de sesión
  ...
  const startListening = () => {
    ...
    // Crear instancia nueva CADA VEZ para limpiar memoria del navegador
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    const recognition = new SpeechRecognition();
    recognition.continuous = false; // CLAVE: False obliga a terminar tras cada frase
    ...
    recognition.onend = () => {
      // Si la bandera sigue activa, reiniciamos inmediatamente (loop infinito controlado)
      if (activeSessionRef.current) {
        // Pequeño delay para no saturar el navegador
        setTimeout(() => {
            if (activeSessionRef.current) {
                try { recognition.start(); } catch(e) { startListening(); }
            }
        }, 100); 
      } else {
    ...
    recognition.onerror = (event) => {
      // Ignoramos errores leves y forzamos reinicio si está activo
      if (event.error === 'no-speech' && activeSessionRef.current) return; 
    ...
    recognition.onresult = (event) => {
      ...
      if (final.trim()) {
        ...
        // Al terminar una frase final, el navegador disparará onend por tener continuous=false
        // y nuestro loop lo reiniciará limpio.
      }
    };
...
export default function RadiologyWorkstation() {
  ...
  // --- DICTADO PC SIMPLE (Toggle Directo) ---
  const togglePcDictation = () => {
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
        return alert("Usa Chrome.");
    }
